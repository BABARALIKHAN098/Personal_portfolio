...
if(process.env.CONTEXT==='production'&&!api)throw new Error('PUBLIC_CHAT_API_URL is required for a production build');
...
if(process.env.CONTEXT==='production'&&!siteUrl)throw new Error('SITE_URL is required for a production build');
