...
def test_secrets_are_not_in_repr():
    settings = Settings(GROQ_API_KEY="secret-one", PINECONE_API_KEY="secret-two", HF_TOKEN="secret-three")
...
def test_production_requires_https_public_origins():
    with pytest.raises(ValueError):
        Settings(APP_ENVIRONMENT="production", GROQ_API_KEY="x", PINECONE_API_KEY="y", HF_TOKEN="z", ALLOWED_ORIGINS="http://localhost:8000", ALLOWED_HOSTS="example.com")


def test_valid_production_configuration():
    settings = Settings(APP_ENVIRONMENT="production", GROQ_API_KEY="x", PINECONE_API_KEY="y", HF_TOKEN="z", ALLOWED_ORIGINS="https://portfolio.example", ALLOWED_HOSTS="api.example")
